...
local function ToggleSpeed(state)
    ...
    if state then speedConnection = R.RenderStepped:Connect(function() SetSpeed(speedValue) end)
    else SetSpeed(16) end
end
        ...
        local function s(p, d)
            ...
            for _, c in ipairs(p:GetChildren()) do
                if c:IsA("Model") and c.Name == "m_Zombie" then
                    ...
                    if zhrp and (zhrp.Position - myPos).Magnitude <= 300 then table.insert(cachedZombiesBG, c) end
                elseif not c:IsA("BasePart") then s(c, d + 1) end
            end
        end
        s(W, 0)
            ...
            if hr then
                ...
                if d <= range then
                    if config.heightCheck then
                        if math.abs(hr.Position.Y - zr.Position.Y) < 5 then nb[#nb+1] = {name=p.Name, dist=d, player=p, hrp=hr} end
                    else nb[#nb+1] = {name=p.Name, dist=d, player=p, hrp=hr} end
                end
            end
        end
    end
        ...
        if z.Parent then
            ...
            if explosivePart then
                ...
                if #nb > 0 then
                    ...
                    if myDist < bs then bs = myDist bz = z bp = explosivePart bn = nb end
                end
            end
        end
    end
...
function fireBG(tool, tp, nb)
    ...
    remote:FireServer("Fire", char:FindFirstChild("Model") or char, tp, W:GetServerTimeNow())
    return true
end
        ...
        if char then
            ...
            if hrp then
                ...
                if tool then
                    ...
                    if bz and bp then fireBG(tool, bp.Position, bn) end
                end
            end
        end
    end
...
local function ToggleBlackGun(state)
    ...
    if state then task.spawn(blackGunLoop) end
end
...
local function createGlowPart(targetPart, color, size)
    ...
    glow.Name = "ESP_Glow" glow.Parent = espFolder
...
local function createESP(zombie)
    ...
    espBoxes[zombie] = {billboard=billboard, textLabel=textLabel, hum=hum, glowParts=glowParts}
    local updateConn = R.RenderStepped:Connect(function()
        if not zombie.Parent or not hum or hum.Health <= 0 then return end
        local cHRP = zombie:FindFirstChild("HumanoidRootPart")
        local cHead = zombie:FindFirstChild("Head") or zombie:FindFirstChild("head")
        if cHRP and glowParts[2] and glowParts[2].Parent then glowParts[2].CFrame = cHRP.CFrame end
        if cHead and glowParts[1] and glowParts[1].Parent then glowParts[1].CFrame = cHead.CFrame end
    end)
    ...
    local function scan(parent)
        for _, child in ipairs(parent:GetChildren()) do
            pcall(function()
                ...
                if n:find("Door") or n:find("Window") or n:find("Glass") then table.insert(result, child) end
            end)
...
local function getPos(obj)
    ...
    if not obj:IsA("Model") then
        ...
        if parent then
            ...
            if parent:IsA("Model") then
                ...
                for _, c in ipairs(parent:GetChildren()) do if c:IsA("BasePart") then return c.Position end end
            end
        end
        return nil
    end
    ...
    if hrp then return hrp.Position end
    for _, c in ipairs(obj:GetChildren()) do if c:IsA("BasePart") then return c.Position end end
    return nil
end
        ...
        if obj.Name == "DoorHit" or obj.Name == "BreakGlass" then
            ...
            if pos then
                pcall(function()
                    ...
                    remote:FireServer("FeedbackStunObject", obj, pos)
                end)
            end
        end
    end
...
task.spawn(function()
    while true do
        task.wait(0.2)
        if destroyEnabled then destroyAll() end
    end
end)

local function ToggleDestroy(state)
    destroyEnabled = state
end

-- ================= 娱乐-ZeroHub =================
local function ToggleZeroHub(state)
    if state then
        pcall(function()
            local a = require(game.ReplicatedStorage.UI.Controller)
            a.BadgeAndPassCheck = function() return true, "N/A" end
            a.ItemCheck = function() return true end
            a.IsAboveCheck = function() return true end
        end)
    end
end
...
local function sendChatMessage(message)
    local remotes = {
        game.ReplicatedStorage:FindFirstChild("DefaultSayMessageRequest"),
        game.ReplicatedStorage:FindFirstChild("SayMessageRequest"),
        game.ReplicatedStorage:FindFirstChild("ChatRequest"),
        game.ReplicatedStorage:FindFirstChild("SendMessage"),
    }
    
    for _, remote in ipairs(remotes) do
        if remote then
            pcall(function() remote:FireServer(message, "All") end)
            pcall(function() remote:FireServer(message) end)
            return true
        end
    end
    ...
    local function findChatRemote(parent)
    ...
    local remote = findChatRemote(game.ReplicatedStorage)
    if remote then
        pcall(function() remote:FireServer(message, "All") end)
        pcall(function() remote:FireServer(message) end)
        return true
...
local Tabs = {
    ...
    Fun = Window:AddTab("娱乐", "gamepad"),
    Rename = Window:AddTab("改名", "edit"),
    Command = Window:AddTab("指令", "terminal"),
}
...
MainGroup:AddLabel("nex制作(侵权即删) nex群1079540447", true)
...
KillGroup:AddToggle("BlackGun", {Text = "普通黑枪", Default = false, Callback = ToggleBlackGun})
KillGroup:AddSlider("BarrelDistance", {Text = "黑枪检测距离", Default = 15, Min = 5, Max = 50, Rounding = 0, Compact = false, Callback = function(v) config.barrelDistance = v end})
KillGroup:AddToggle("HeightCheck", {Text = "高度检测", Default = false, Callback = function(v) config.heightCheck = v end})
...
AimBox:AddToggle("AimSmooth", {Text = "平滑自瞄", Default = false, Callback = function(v) K.Aim.Sm = v end})
AimBox:AddSlider("AimSpeed", {Text = "平滑速度", Default = 15, Min = 1, Max = 100, Rounding = 0, Callback = function(v) K.Aim.Sp = v / 100 end})
AimBox:AddToggle("AimPred", {Text = "预判自瞄", Default = false, Callback = function(v) K.Aim.Pd = v end})
AimBox:AddSlider("AimPredStr", {Text = "预判强度", Default = 20, Min = 1, Max = 100, Rounding = 0, Callback = function(v) K.Aim.PS = v / 100 end})
...
-- ================= 娱乐UI =================
local FunGroup = Tabs.Fun:AddLeftGroupbox("娱乐功能", "gamepad")
FunGroup:AddToggle("DestroyDoors", {Text = "全图自动拆门", Default = false, Callback = ToggleDestroy})
FunGroup:AddToggle("ZeroHub", {Text = "解锁全军团", Default = false, Callback = ToggleZeroHub})

-- ================= 改名UI =================
local RenameGroup = Tabs.Rename:AddLeftGroupbox("改名功能", "edit")

local playerDropdown = RenameGroup:AddDropdown("PlayerSelect", {
    Text = "选择玩家",
    Values = {},
    Default = "",
    Callback = function(v)
        renameSelectedFolder = v
    end,
})

local function refreshRenameList()
...
local renameInput = RenameGroup:AddInput("NewName", {
    Text = "新文件夹名称",
    Placeholder = "输入新名称",
    Default = "",
})
...
RenameGroup:AddButton({
    Text = "修改名称",
    Func = function()
        if renameSelectedFolder == "" then return end
        local newName = renameInput.Value or ""
        if newName == "" then return end
        local targetFolder = workspace.Players:FindFirstChild(renameSelectedFolder)
        ...
        if ok then
            renameSelectedFolder = newName
            refreshRenameList()
        end
    end,
})

RenameGroup:AddLabel("仅显示活着的玩家文件夹", true)

task.spawn(function()
    while true do
        task.wait(2)
        pcall(refreshRenameList)
    end
end)

-- ================= 指令UI =================
local CommandGroup = Tabs.Command:AddLeftGroupbox("指令发送", "terminal")

local commandInput = CommandGroup:AddInput("CommandText", {
    Text = "指令内容",
    Placeholder = "输入指令，如 /kill 玩家名",
    Default = "",
})
...
CommandGroup:AddButton({
    Text = "发送指令",
    Func = function()
        local msg = commandInput.Value or ""
        if msg == "" then return end
        sendChatMessage(msg)
    end,
})

CommandGroup:AddLabel("输入 / 开头的指令", true)
